...
```{r}
games <- read_excel("../data/spil_unique_ID_Title_platform_collapsed.xlsx")
```
...
```{r}
games %>% 
  count(Platforme, sort = TRUE) %>% 
  slice_max(n, n = 15) %>% 
  mutate(Platforme = reorder(Platforme, n)) %>%
  ggplot(aes(x = Platforme, y = n)) +
  geom_col() +
  coord_flip()
```
...
```{r}
games_title_text_tidy <- games %>% 
  unnest_tokens(word, Titel)
```
...
```{r}
games %>% 
  filter(str_detect(Titel, "Skolehjælp"))
```
...
```{r}
games %>% 
  filter(str_detect(Titel, "toddlers"))
```
